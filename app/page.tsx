...
export default function Home() {
  ...
  const send = async () => {
    ...
    const title = activeConversation.title === "新对话" ? user.parts[0].text.slice(0, 22) : activeConversation.title; await dbRef.current.conversations.update(activeConversation.id, { title, updatedAt: now }); await refresh(activeConversation.id);
